...
Equations or sections in this code refer to the paper to be found at
https://arxiv.org/abs/2005.11110

Note that in the code several inversion-like operations (i.e. Cholesky decompositions
or matrix solves) on moderately large batches of very small matrices have to be performed.
Naively executed on a GPU, those operations are very slow so they are shifted
to the CPU by default (along with the respective gradient computations,
see '../experiments/utils/optimise.py').
"""
...
class Fully_Coupled_Layers(Layers):
    ...
    def sample_from_first_layer(self, X,S, z=None):
        ...
        only the actual sampling from the resulting distribution has to be repeated.
    ...
    def SlNtilde(self,X,l,Tl,KMM,KMN_inv,startind = 0,stopind = 0):
        ...
        Thm. 1, partly explained in Appx. E.1.
        """
        if l == 0:
            #calculate S_M^11
    ...
    def draw_samples_first_layer(self,N,S,T1,mean,var,z):
        ...
        #one of the operations that is slow on GPUs
        with tf.device("/cpu:0"):
            SN1tilde_chol = tf.cholesky(var + I_jitter) #tf addition broadcasts, N,T1,T1
        if z is None:
    ...
    def SlNtildesubT(self,l,Tl,KlMN_inv,all_KMN_inv,startind = 0,stopind = 0):
        """
        Calculate the quantity in Eq. (115), or rather its transposed as explained in Appx. E.1.
        """
    ...
    def update_cholesky(self,Tl,SNtilde_chol,SlNtildesubT,SlNtilde):
        ...
        to obtain those of \widetilde{S}_n^{1:l,1:l} as explained in the second part of Appx. E.1.
        """
        ...
        S21 = tf.matmul(L21T,L21T,transpose_a = True)
        #one of the operations that is slow on GPUs
        with tf.device("/cpu:0"):
            L22 = tf.cholesky(SlNtilde - S21 + I_jitter) # N,Tl,Tl
...
class Fully_Coupled_Sampling_Layers(Fully_Coupled_Layers):
    def __init__(self, X, Y, Z, all_kernels, all_mfs, all_Zs, mu_M=None, S_M=None,
                 whitened_prior=False, **kwargs):
        Fully_Coupled_Layers.__init__(self, X, Y, Z, all_kernels, all_mfs, all_Zs, mu_M=mu_M, S_M=S_M,
                                      is_full=True, whitened_prior=whitened_prior, **kwargs)
    
    """
    The major change to the analytical marginalization method is how samples f_N^l
    from intermediate layers are obtained.
    That is, by drawing a sample from p(f_N^l|f_M^l; X), where the f_M^l have
    been sampled before.
    """
    def sample_from_lth_layer(self,l, X, f_M, S, z=None):
        
        """
        Inputs:
        l, zero-based layer index
        X (S*N, T_{l-1}), f_N^{l-1} samples drawn in previous layer
        f_M (Tl,S,M,1), f_M^l samples drawns from the joint distribution q(f_M)
        S, number of MC samples to be drawn
        (optional) z (S*N,T_l), list of values used for the reparametrization trick for reproducibility
        
        Outputs:
        layer_samples (S*N, T_l) samples drawn in this layer
        layer_mean (S*N,T_l), means used to draw the samples of the current layer
        layer_var (S*N,T_l), variances used to draw the samples of the current layer
        """
        
        Tl = self.all_Gps_per_layer_tf[l] #nof latent processes in lth layer
        
        """calculate quantities for p(f_N^l|f_M^l; X) according to after Eq. 2"""
        
        """first KMN_inv, that is \tilde{K}^l_{MN} according to Eq. 1"""
        self.build_cholesky_if_needed()        
        #KMM = self.all_KMM[0]
        LMM = self.all_LMM[l]        
        KMN = self.all_features[l].Kuf(self.all_kernels[l],X)
        if not self.whitened_prior:
            KMN_inv = tf.cholesky_solve(LMM, KMN) #matmul(KMM_inv,KMN) #M,N
        else:
            KMN_inv = tf.matrix_triangular_solve(LMM, KMN,lower=True) #matmul(LMM_inv,KMN) #M,N
            
        """calculate mean (\tilde{K}^l_{NM} x f_M) """
        #care has to be taken with transposing KMN for a correct matrix multiplication
        #and that the multiple samples S are treated correctly
        #(note that for this part S is explicitly denoted, before it was absorbed in N)
        KNM_inv_t = tf.transpose(KMN_inv,(1,0)) #S*N,M
        KNM_inv = tf.reshape(KNM_inv_t,[S,-1,self.M])#S,N,M
        KNMtilde = tf.tile(KNM_inv[None,:,:,:],(Tl,1,1,1))#Tl,S,N,M
        #note that f_M is of shape (Tl,S,M,1)
        layer_mean = tf.matmul(KNMtilde,f_M) #Tl,S,N,1
        layer_mean = tf.transpose(tf.reshape(layer_mean[:,:,:,0],[Tl,-1]),(1,0)) #N*S,Tl
        #(note that in the following, once again S absorbed in N)
        
        layer_mean += self.all_mean_funcs[l](X) #N,Tl (add untrainable mean function)
        
        """ calculate variance (\tilde{K}^l_{NN} in Eq. 1)"""
        KlNN_tiled = tf.tile(self.all_kernels[l].Kdiag(X)[:,None],(1,Tl)) #N,Tl
        if not self.whitened_prior:
            #diagonal of KNM @ KMM_inv @ KNM.T can be obtained by
            #reduce_sum(KNM @ KMM_inv * KNM.T)
            temp = tf.multiply(KMN,KMN_inv) #M,N
            temp = tf.tile(tf.reduce_sum(temp,axis=0)[:,None],(1,Tl)) #N,Tl
        else:
            #diagonal of KNM @ KMM_inv @ KNM.T can be obtained by using
            #KNM @ KMM_inv @ KNM.T = (LMM_inv @ KNM.T).T @ (LMM_inv @ KNM.T).
            #its diagonal is obtained by reduce_sum((LMM_inv @ KNM.T) * (LMM_inv @ KNM.T))
            temp = tf.multiply(KMN_inv,KMN_inv) #M,N
            temp = tf.tile(tf.reduce_sum(temp,axis=0)[:,None],(1,Tl)) #N,Tl
        layer_var = KlNN_tiled - temp #N,Tl
        
        if z is None:
            z = tf.random_normal([tf.shape(X)[0],Tl], dtype=settings.float_type) #N,Tl        
        
        """finally sample from the resulting multivariate Gaussian"""
        # note that the dependence between the samples for different t= 1, ..., Tl
        # is invisible here. but it is still there as this dependence is encoded
        # in the sample of f_M that has been drawn (outside of this function)
        layer_samples = layer_mean + z * (layer_var + settings.jitter) ** 0.5 #N,Tl
        
        return layer_samples, layer_mean, layer_var
...
class Fast_Stripes_Arrow_Layers(Fully_Coupled_Layers):
    ...
    \widetilde{S}_n^{1:L,1:L} is exploited. The latter point is explained in more detail in Appx. E.2.
    ...
    in the second to last paragraph of Appx. E.2.
    """
    ...
    def SlNtilde(self,X,l,Tl,KMM,KMN_inv,startind = 0,stopind = 0):
        ...
        where Eqs. 128 and 129 are used.
        Having obtained S_M^ll, in the second part the S_N^l are obtained with the same idea as for full S_M.
        Since only the diagonal elements are non-zero, some simplifications can be made,
        (see also the code in https://github.com/ICL-SML/Doubly-Stochastic-DGP/ )"""
        if l == 0:#first layer is always diagonal
            mat = self.diag_blocks[:Tl] #T1,M,M
        elif l == self.L-1: #last layer (depends on arrow), according to Eq. 129
            if self.arrow:
        ...
        else: #intermediate layers depend on stripes, according to Eq. 128
    ...
    def SlNtildesub_last_layer(self,l,KlMN_inv,all_KMN_inv):
        ...
        The basic idea is to use the equivalent Eq. 116 and exploit that the terms
        used for the last layer can be efficiently obtained using Eq. 131.
        """
    ...
    def SlNtildesub_intermediate_layer(self,l,KlMN_inv,all_KMN_inv):
        """
        special implementation, calculating the terms of \widetilde{S}_n^{l,1:l}.
        The basic idea is to use Eq. 116 and exploit the sparsity using Eq. 130.
        """
